...
public class ParseFromDatModel1UpToDate {	
	...
	private static void getAddTagL(JSONArray xputs, String address, boolean isInput) throws JSONException{
		...
		ParseFromDatModel1UpToDate.addrSet.remove(new Address(address, null, null));
		
		// no addr entry or no correct addr entry
		if(!(item.has("addr") && item.get("addr").toString().equals(address))){
			ParseFromDatModel1UpToDate.addrSet.add(new Address(address, null, null));
			return;
		}
		if (item.has("addr_tag_link") || item.has("addr_tag")) {
			if (item.has("addr_tag_link") && item.has("addr_tag")) {
				System.out.println(address);
				ParseFromDatModel1UpToDate.addrSet.remove(new Address(address, item.get("addr_tag_link").toString(), null));
				ParseFromDatModel1UpToDate.addrSet.remove(new Address(address, null, item.get("addr_tag").toString()));
				ParseFromDatModel1UpToDate.addrSet.add(new Address(address, item.get("addr_tag_link").toString(), item.get("addr_tag").toString()));
			} else if (item.has("addr_tag_link")) {
				System.out.println(address);
				ParseFromDatModel1UpToDate.addrSet.add(new Address(address, item.get("addr_tag_link").toString(), null));
			} else {
				System.out.println(address);
				ParseFromDatModel1UpToDate.addrSet.add(new Address(address, null, item.get("addr_tag").toString()));
			}
		}else{
			ParseFromDatModel1UpToDate.addrSet.add(new Address(address, null, null));						
		}
	...
	public static void main(String[] args) throws IOException, JSONException {
		...
		Address adr1 = new Address("17jCUUdXVg8eNr5J6x7Rq13f3LcoZ2SP3b", null, null);
		Address adr2 = new Address("17jCUUdXVg8eNr5J6x7Rq13f3LcoZ2SP3b", null, null);
...
//								addrSet.add(multiAddList.toString());
								...
								inputStr.append(',');
								double temp = ParseFromDatModel1UpToDate.getDollarVal(ta.get("time").toString(), inp.get("value").toString());
								if (temp == 0.0){
									return;
								}
								...
								ParseFromDatModel1UpToDate.getAddTagL(inputsArr, inp.get("address").toString(), true);
					...
					for(int j = 0; j < outps.length(); j ++){
						...
						if(!outp.get("type").equals("op_return")){
							if(outp.get("type").equals("multisig")){
								...
								outputStr.append(',');
								double temp = ParseFromDatModel1UpToDate.getDollarVal(ta.get("time").toString(), outp.get("value").toString());
								if(temp == 0.0){
									return;
								}
								...
								ParseFromDatModel1UpToDate.getAddTagL(outs, outp.get("address").toString(), false);
